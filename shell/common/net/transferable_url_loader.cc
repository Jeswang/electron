...
TransferableURLLoader::TransferableURLLoader(
    Delegate* delegate,
    scoped_refptr<network::SharedURLLoaderFactory> upstream_factory,
    const GURL& initial_url)
    : delegate_(delegate),
      upstream_factory_(std::move(upstream_factory)),
...
std::optional<PendingURLLoaderResponse> TransferableURLLoader::TakeResponse() {
  ...
  auto endpoints = network::mojom::URLLoaderClientEndpoints::New(
      upstream_loader_.Unbind(), upstream_client_receiver_.Unbind());
...
void TransferableURLLoader::CreateLoaderAndStart(
  ...
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  CHECK(!downstream_loader_receiver_.is_bound());
  downstream_loader_receiver_.Bind(std::move(loader));
  downstream_client_.Bind(std::move(client));
  upstream_factory_->CreateLoaderAndStart(
      upstream_loader_.BindNewPipeAndPassReceiver(), request_id, options,
      request, upstream_client_receiver_.BindNewPipeAndPassRemote(),
      traffic_annotation);
  upstream_client_receiver_.set_disconnect_handler(base::BindOnce(
      &TransferableURLLoader::OnUpstreamDisconnect, base::Unretained(this)));
}

void TransferableURLLoader::Clone(
    mojo::PendingReceiver<network::mojom::URLLoaderFactory> receiver) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  upstream_factory_->Clone(std::move(receiver));
}

std::unique_ptr<network::PendingSharedURLLoaderFactory>
TransferableURLLoader::Clone() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  return upstream_factory_->Clone();
}
...
void TransferableURLLoader::FollowRedirect(
  ...
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  upstream_loader_->FollowRedirect(std::move(headers_update_params), new_url);
}

void TransferableURLLoader::SetPriority(net::RequestPriority priority,
                                        int32_t intra_priority_value) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  upstream_loader_->SetPriority(priority, intra_priority_value);
}

void TransferableURLLoader::OnReceiveEarlyHints(
    network::mojom::EarlyHintsPtr early_hints) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  CHECK(downstream_client_.is_bound());
  downstream_client_->OnReceiveEarlyHints(std::move(early_hints));
}
...
void TransferableURLLoader::OnReceiveRedirect(
  ...
  downstream_client_->OnReceiveRedirect(redirect_info, std::move(head));
}
...
void TransferableURLLoader::OnUploadProgress(
  ...
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  CHECK(downstream_client_.is_bound());
  downstream_client_->OnUploadProgress(current_position, total_size,
                                       std::move(callback));
}
...
void TransferableURLLoader::OnTransferSizeUpdated(int32_t transfer_size_diff) {
  ...
  if (!response_received_) {
    CHECK(downstream_client_.is_bound());
    downstream_client_->OnTransferSizeUpdated(transfer_size_diff);
...
void TransferableURLLoader::OnComplete(
  ...
  if (!response_received_) {
    CHECK(downstream_client_.is_bound());
    delegate_ = nullptr;
    downstream_client_->OnComplete(status);
    return;
  }
...
void TransferableURLLoader::OnUpstreamDisconnect() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  if (completion_status_)
    return;
